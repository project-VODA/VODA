...
- 23.08.10 KJW
  - color 페이지 제작
  - 화면 캡쳐해서 axios로 flask에 이미지 전송 후 인식된 색상 받아오기 
...
- 23.08.14 KJW
  - color page 사용법 추가
  - 색상 인식 페이지 모바일 화면 고려하여 CSS 수정
  - 표정에 관한 조언 추가
  - 표정 데이터 expression, probability 리덕스로 관리하도록 수정
...
- 23.08.15 KJW
  - VideoRoomComponent에서 typeNo 못받아 오는 문제 해결 (오타 수정)
  - 환경 설정에서 조언 선택한 경우 표정 듣기가 안되는 문제 해결

- 23.08.16 KSH
  - 통화 거절시 모달 나가기 버튼 가운데로 수정
  - simple about -> 표정 -> 세상
  - 비디오 반전 주석처리(추후 주석만 풀면 변경)

- 23.08.16 KJW
  - 색상이 undefined일 때 관련 안내 추가
  - 얼굴이 인식 되지 않았을 때 관련 안내 추가
  - 심플 모드 Color Page 버튼 변경

- 23.08.17 KSH
  - 심플 고객의 소리 상세조회 페이지 글 내용 스크린 리더기가 인식 못하는 문제 해결
  - 디테일 모드 네비게이션에서 고객의 소리와 색상 인식 순서 변경
  - 심플 모드 버튼 위치 재배열 (기능 윗줄 / 부가 아랫줄)
  - 서비스 이용약관 레이아웃 변경
  - 디테일 홈페이지 캐러셀에 색상인식 추가
  - 약간의 모바일 환경 개선(로그인 페이지라던가)
  - 디테일 어바웃 페이지(gif 추가)

- 23.08.17 KJW
  - openvidu 상대방 화면 좌우 반전

- 23.08.19 KJW
  - 디테일 모드 및 심플 모드 서비스 소개 페이지 수정
  - tss 적용 전 사용했던 표정 audio 데이터 삭제
  - 디테일 모드 환경 설정 심플 모드와 동일하게 수정

- 23.08.20 KJW
  - 색상 인식 페이지에 전체 화면 캡쳐 추가
  - flask에 /cosmetics로 axios 요청 추가
  - 영상 통화 시 모드 토글 버튼 안보이게 처리